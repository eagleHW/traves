...
public class PViajesCancelados extends Fragment implements View.OnClickListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        Cursor c = bd.obtener_cancelados(settings.getString("login","ERROR"));


        textViews = new TextView[c.getCount()][c.getColumnCount()];
        buttons = new Button[c.getCount()];

        View view = inflater.inflate(R.layout.fragment_pviajes_cancelados, container,false);
        LinearLayout principal = (LinearLayout) view.findViewById(R.id.llp_viajes_cancelados);
        ...
        while ( c.moveToNext()){
            ...
            buttons[num_reg].setText("Cancelado");
            buttons[num_reg].setAllCaps(false);
            buttons[num_reg].setGravity(Gravity.CENTER);
            buttons[num_reg].setLayoutParams(params);
            buttons[num_reg].setTextColor(Color.rgb(255,0,0));
            buttons[num_reg].setBackground(getResources().getDrawable(R.drawable.trasparent_button));
